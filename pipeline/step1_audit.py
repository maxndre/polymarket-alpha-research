"""
step1_audit.py – ÉTAPE 1 : Cartographie et audit de la base (Data Mapping).
...
# ─────────────────────────────────────────────────────────────────────────────
# Helper: parse tags_json column into flat list of labels (vectorized)
# ─────────────────────────────────────────────────────────────────────────────
...
def _explode_tags(df_markets: pd.DataFrame) -> pd.DataFrame:
    ...
    Uses pd.json_normalize inside a vectorized apply on strings only.
    """
    ...
    # Vectorized string parsing – apply only on the column (Series), not rows
...
def map_and_audit_database(
    ...
    """
    ÉTAPE 1 – Cartographie et audit de la base.

    Parameters
    ----------
    df_markets    : market table (58 k rows).
    df_bars       : trade_price_1m table (10 M rows).
    df_token      : token_outcome table (116 k rows).
    df_market_tag : market_tag table (392 k rows).

    Returns
    -------
    Nested dict report suitable for JSON serialisation.
    """
    logger.info("=== ÉTAPE 1 : CARTOGRAPHIE ET AUDIT DE LA BASE ===")
    report: dict = {}
    ...
    logger.info("Bar rows (1-min OHLCV): %d", n_bars)

    # Tags
    df_tags_long = _explode_tags(df_markets)
    tag_counts   = (df_tags_long.groupby("tag_label")["condition_id"]
                                .nunique()
                                .sort_values(ascending=False))
    top_tags = tag_counts.head(30).to_dict()

    # Tag-bucket breakdown via market_tag
    bucket_counts = (df_market_tag.groupby("tag_slug")["condition_id"]
                                  .nunique()
                                  .sort_values(ascending=False)
                                  .head(20)
                                  .to_dict())
    ...
    report["overview"] = {
        ...
        "top_30_tags_by_market_count": {str(k): int(v) for k, v in top_tags.items()},
    ...
    duration_h = (end - start).dt.total_seconds() / 3600.0  # hours

    report["temporal"] = {
        "trade_bar_date_min": str(df_bars["minute_ts"].min()),
        "trade_bar_date_max": str(df_bars["minute_ts"].max()),
        "market_start_min":   str(start.min()),
        "market_start_max":   str(start.max()),
        "market_end_min":     str(end.min()),
        "market_end_max":     str(end.max()),
    ...
    def _missing_rates(df: pd.DataFrame) -> dict:
        ...
        if total == 0:
            return {}
        rates = (df.isnull().sum() / total * 100).round(2)
    ...
    # Prices outside [0, 1]
    def _pct_out_of_bounds(series: pd.Series, lo: float = 0.0, hi: float = 1.0) -> float:
        valid = series.dropna()
        if len(valid) == 0:
            return np.nan
    ...
    for col in price_cols:
        ...
        price_anomalies[col] = {
            ...
            "pct_inf":          float(np.isinf(s.replace([None], np.nan).fillna(0)).mean() * 100),
            "pct_out_of_01":    _pct_out_of_bounds(s),
            "pct_negative":     float((s.dropna() < 0).mean() * 100),
        }

    # Intra-bar spread anomalies: high < low is structurally impossible
    if {"high_price", "low_price"}.issubset(df_bars.columns):
        negative_spread = (df_bars["high_price"] < df_bars["low_price"]).sum()
        zero_spread     = (df_bars["high_price"] == df_bars["low_price"]).sum()
    else:
        negative_spread = zero_spread = -1

    # Bars with trades_count > 0 but missing prices
    active_bars   = df_bars[df_bars["trades_count_1m"] > 0]
    n_active_bars = len(active_bars)
    active_missing_prices = int(active_bars[price_cols].isna().any(axis=1).sum())

    # Timestamp inversion check (market level): end_date < start_date
    ...
    report["microstructure_anomalies"] = {
        ...
        "n_bars_high_lt_low":        int(negative_spread),
        "n_bars_high_eq_low":        int(zero_spread),
        "n_active_bars_total":       n_active_bars,
        "n_active_bars_missing_price": active_missing_prices,
        "n_markets_ts_inverted":     ts_inverted,
    }

    # ── 1.5 Activity distribution across markets ──────────────────────────────
    ...
    # Zero-activity markets
    n_zero_trades  = int((per_market["total_trades"]  == 0).sum())
    n_zero_volume  = int((per_market["total_volume_usdc"] == 0).sum())
    report["activity_summary"]["n_markets_zero_trades"] = n_zero_trades
    report["activity_summary"]["n_markets_zero_volume"] = n_zero_volume

    logger.info("Audit complete. %d anomaly types detected.", len(report["microstructure_anomalies"]))
